...
public class EpidemicQASentenceCollectionTest  extends DocumentCollectionTest<EpidemicQASentenceCollection.Document> {
  @Before
  public void setUp() throws Exception {
    super.setUp();

    collectionPath = Paths.get("src/test/resources/sample_docs/epidemic_qa");
    collection = new EpidemicQASentenceCollection(collectionPath);
  ...
  void checkDocument(SourceDocument doc, Map<String, String> expected) {
    EpidemicQASentenceCollection.Document covidSentence = (EpidemicQASentenceCollection.Document) doc;

    assertEquals(expected.get("id"), covidSentence.id());
    assertTrue(covidSentence.contents().startsWith(expected.get("contents_starts_with")));
    assertTrue(covidSentence.contents().endsWith(expected.get("contents_ends_with")));
    assertEquals(Integer.parseInt(expected.get("contents_length")), covidSentence.contents().length());

    // Make sure raw() is a JSON containing proper fields, and check length.
    ObjectMapper mapper = new ObjectMapper();

    try {
      JsonNode jsonNode = mapper.readTree(covidSentence.raw());
    ...
    assertEquals(Integer.parseInt(expected.get("raw_length")), covidSentence.raw().length());
  }
}
